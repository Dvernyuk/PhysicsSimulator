...
public class Main {
	...
	private static Options buildOptions() {
		...
		//mode
		cmdLineOptions.addOption(Option.builder("m").longOpt("mode").hasArg().desc("Execution Mode. Possible values: 'batch' (Batch mode), 'gui' (Graphical User Interfacemode). Default value: '"+_modeDefaultValue+"'.").build());
	...
	private static void parseStepsOption(CommandLine line) throws ParseException {
		int s = 0;
		if(line.hasOption("s") && _mode == "batch") {
			s = Integer.parseInt(line.getOptionValue("s"));
			if (s <= 0)
				throw new ParseException("Incorrect number of simulation steps: "+s);
			 else
				_steps = s;
		}
	}
	...
	private static void parseOutFileOption(CommandLine line) throws ParseException {
		if(line.hasOption("o") && _mode == "batch") {
